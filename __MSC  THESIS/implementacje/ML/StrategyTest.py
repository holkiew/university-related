...
if __name__ == '__main__':
    ...
    for stock in stocks:
        for z in range(10):
            ...
            xy = features[0]
            baseValue = features[1].broker.getvalue()
            X, y  = list(), list()
            for val in xy.values():
                # ficzersy
                X.append(val[0])
                # 0-nic, 1 buy, -1 sell
                y.append(val[1])
            ...
            saveToFile(f"data/results/random_close/{stock}/{stock}_R(50_501_50)V_%.0f_TREE_MAX.txt" % (baseValue/1000), str().join(fileTxt))
            fileTxt.clear()
