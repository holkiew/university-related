...
class MomentumStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        for dataset in self.datas:
            self.indicators[dataset._name] = GARR(dataset)
    ...
    def next(self):
        ...
        for k in self.indicators:
            stockDict[k] = self.indicators[k][0]

        # ordered stocks by value
        orderedStocks = OrderedDict(sorted(stockDict.items(), key=lambda x: x[1], reverse=True))
        counter = 1
