...
requires y != 0 //weakest precondition

<<<<<<< HEAD
{
    return x/y;
}
//Returns the the square of x, i.e., x * x
method square(x:int) returns (z:int)
//no errors means weakest precondition is true
requires true;
ensures z == x*x;    //strongest postcondition
{   
    assert x * x >=0;
    return x*x;
}
...
/**
    ...
    the second one.
>>>>>>> 0179cdd14aed6ca785d3c6d2e6f0aad7c61c4002
...
requires y != 0;
ensures z == x % y;
ensures z + x/y * y == x;  //same as percentage
{
    return x % y;  
}
...
requires true;
ensures z == -1 || z == 1; //moja wersja
ensures z == 1 <==> x >= 0; //jego wersje
ensures z == -1 <==> x < 0; //jego
{
    if(x >= 0){
        return 1;
    }else {
...
<<<<<<< HEAD
//Returns the sum of all natuiral numbers plus zero up to n
method computeSumallpositive(x:int) returns (z:int)
//n == co x
requires x >= 0;
ensures z == sumallpositive(x); //to samo
ensures z == x* (x+1)/2;
{
    var sum := 0;
    var i := 0;

    while (i < x) //0 <= x - i bez znaku rownosci <= bo wtedy od razu mamy loop 0
        decreases x - i
        invartiant 0 <= i <= x
        invartiant sum == sumallpositive(i) 
    {
        //loop invartians ex. for 0 loops, 1 or more
        i := i + 1;
...
<<<<<<< HEAD
//Inductive definiton of factorial
